...
pub struct CommentView {
    ...
    pub comment_content: String,
    pub reply_content: Option<CommentReplyView>,
}
...
impl FromRow for CommentView {
    ...
    ///        cut.reply_to, cut2.user_name, cut.user_nickname, cut2.avatar, cut2.comment_content
    /// ```
    ///
    /// `cut.comment_id, cut.user_name, cut...` is this comment information.
    ///
    /// `cut.reply_to, cut2.user_name, cut.user_ni...` is comment what this comment reply to.
        ...
        Self: Sized,
    {
        ...
        Ok(Self {
            ...
            reply_content: match row[6] != Value::NULL {
                true => Some(CommentReplyView {
                    comment_id: get_value(&row[6], &row)?,
                    user: UserSimplify {
                        user_name: get_value(&row[7], &row)?,
                        user_nickname: get_value(&row[8], &row)?,
                        avatar: get_value(&row[9], &row)?,
                    },
                    comment_content: get_value(&row[10], &row)?,
                }),
                false => None,
            },
        })
...
impl CommentView {
    pub fn get_comments(post_id: i32, offset: i32, count: i32) -> Result<Vec<Self>, DaoError> {
        ...
        let sql = r"select cut.comment_id, cut.user_name, cut.user_nickname, cut.avatar, cut.comment_time, cut.comment_content, 
            cut.reply_to, cut2.user_name, cut.user_nickname, cut2.avatar, cut2.comment_content
        from (
          select ct.comment_id, ct.post_id, ct.user_id, ct.reply_to, ct.comment_time, ct.comment_content, ut.user_name, ut.user_nickname, ut.avatar 
          from comment_table ct 
          left join user_table ut on ct.user_id=ut.user_id
        ) cut
        left join (
          select ct.comment_id, ct.comment_content, ut.user_name, ut.user_nickname, ut.avatar 
          from comment_table ct 
          left join user_table ut on ct.user_id=ut.user_id
        ) cut2 on cut2.comment_id=cut.reply_to
...
impl CommentPutRequest {
    pub fn put_comment(&self) -> Result<(), DaoError> {
        ...
        let sql = r"insert comment_table(post_id,user_id,reply_to,comment_time,comment_content)
            values (:post_id, 
